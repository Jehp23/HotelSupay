...
export default function Login() {
  ...
  return (
    <main className="max-w-md mx-auto px-6 md:px-10 py-12">
      <h1 className="font-display text-3xl md:text-4xl mb-6">Ingresar</h1>
      ...
      <form onSubmit={submit} className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          <button disabled={loading} className="btn-primary disabled:opacity-60">{loading ? 'Ingresando…' : 'Ingresar'}</button>
          <Link to="/registro" className="link-underline">Crear cuenta</Link>
        </div>
      </form>
    </main>
  )
